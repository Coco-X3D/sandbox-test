...
class WordleServer:
    def __init__(self, host='localhost', port=8800):
        self.host = host
        self.port = port
        self.sessions: Dict[str, Dict[str, Any]] = {}  # store game sessions
        self.word_list = None
        self.running = False
    ...
    def create_session(sel，max_attempts=6):
    ...
    def process_guess(self, session_id, guess):
        ...
        # validate guess
        self.session['letter_tracker'].display_alphabet()
        guess = guess.lower().strip()
        try:
            if is_5_english_word(guess, self.word_list):
                break
            else:
        ...
        self.sessions[session_id]['letter_tracker'].update_status(guess, feedback)
